...
namespace BackOffice.Models
{
    public class RazorPageAsPdf : AsPdfResultBase
    {
        ...
        protected override async Task<byte[]> CallTheDriver(ActionContext actionContext)
        {
            ...
            // Remove the component with id="excludeFromPdf"
        ...
        private string RemoveExcludedElements(string html)
        {
            // Regex to match and remove elements with id="excludeFromPdf"
            string pattern = @"<[^>]*id\s*=\s*[""']excludeFromPdf[""'][^>]*>.*?</[^>]+>";
            return Regex.Replace(html, pattern, string.Empty, RegexOptions.IgnoreCase | RegexOptions.Singleline);
        }
    }
}
